        ...
        <div style={{width: isOpen ? "200px" : "50px"}} className={styles.Navbar}>
          ...
          </div>
          {
            Items.map((item, index)=>(
              <NavLink id={item.id} to={item.url} key={index} className={styles.link}>
                <div className={styles.icon}>{item.icon}</div>
                <div style={{display: isOpen ? "block" : "none"}} className={styles.link_text}>{item.title}</div>
              </NavLink>
            ))
          }
          <NavLink to="/login" onClick= {logoutHandler}className={styles.link}>
                <div className={styles.icon}><GrLogout/></div>
                <div style={{display: isOpen ? "block" : "none"}} className={styles.link_text}>로그아웃</div>
          </NavLink>
        </div>
      </div>
    );
